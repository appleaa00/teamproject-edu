...
public class SysOssServiceImpl implements ISysOssService, OssService {
  ...
  public SysOssVo upload(MultipartFile file) {
    ...
    try {
      ...
      // OSS配置出错时，使用本地文件存储作为备选方案
      log.warn("OSS上传失败，使用本地存储: {}", e.getMessage());
      return uploadToLocal(file, originalfileName, suffix);
    }
  }
   ...
   * 本地文件存储备选方案
   */
  private SysOssVo uploadToLocal(MultipartFile file, String originalfileName, String suffix) {
    try {
      // 生成唯一文件名
      String fileName = System.currentTimeMillis() + suffix;
      
      // 直接使用固定的上传目录（项目根目录）
      String uploadDir = "C:" + java.io.File.separator + "Users" + java.io.File.separator + "28279" + 
                        java.io.File.separator + "Desktop" + java.io.File.separator + "SourceCode" + 
                        java.io.File.separator + "uploads" + java.io.File.separator + "images" + java.io.File.separator;
      
      log.info("使用上传目录: {}", uploadDir);
      ...
      // 保存文件到本地
      java.io.File localFile = new java.io.File(uploadDir + fileName);
      ...
      throw new ServiceException("本地文件存储失败: " + e.getMessage());
    }
  }
