...
public class SysRegisterService {
    ...
    public void register(RegisterBody registerBody) {
        ...
        if(StringUtils.isNotBlank(registerBody.getTenantId())){
            tenantId = registerBody.getTenantId();//注册时既支持默认租户，也支持指定租户注册
        }
        String username = registerBody.getUsername();//来自前端 JSON 的用户输入
        ...
        sysUser.setUserBalance(1.0);//设置初始用户余额
    ...
    public void resetPassWord(RegisterBody registerBody) {
        ...
        String password = registerBody.getPassword();
        SysUserVo user = userService.selectUserByUserName(username);//从数据库里查出这个用户是否存在
        if(user == null){
            throw new UserException(String.format("用户【%s】,未注册!",username));
        }
        // 检查验证码是否正确
        validateEmail(username,registerBody.getCode());
        userService.resetUserPwd(user.getUserId(),BCrypt.hashpw(password));//数据库层更新
    }
