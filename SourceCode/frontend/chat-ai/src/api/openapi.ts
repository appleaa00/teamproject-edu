...
export const whisperUpload = ( FormData:FormData )=>{
    const url = gptGetUrl('/chat/audio'); // 请求地址
    let headers=   {'Content-Type': 'multipart/form-data','Authorization':'Bearer ' + getToken()} // 请求头
    headers={...headers,...getHeaderAuthorization()}
    return new Promise<any>((resolve, reject) => {
            axios.post( url , FormData, {
            headers  //使用 axios 发送 POST 请求，参数分别是：请求 URL、包含音频文件的 FormData、请求头配置
        }).then(response =>  resolve(response.data )  
        ).catch(error =>reject(error)  ); 
    })
}
