...
// 添加导出教案函数
const exportTeachingPlan = async () => {
  try {
    // 显示加载提示
    message.loading({
      content: '正在生成教案截图...',
    ...
    link.download = `智能教学方案_${courseTopic.value}_${new Date().toLocaleDateString().replace(/\//g, '-')}.png`;
...
生成内容1：教学方案概览，总时长90分钟，包括理论讲解，实践操作和互动环节，自己根据实际课程类型去合理分配时间，结果都加上分钟
...
最终结果以JSON格式返回，jxfa为教学方案概览，对象类型，其中：lt为总时长，lljj为理论讲解，sjcz为实践操作，hdhj为互动环节；zsd为知识点结构其中：zt为主题，nr为内容，sc为讲解时间；sxal为实训案例，其中zt为主题，gs为概述，mb为实训目标，gc为实训过程；hdnr为互动内容，其中：zt为主题，nr为表示问题、任务或竞赛主题，所有时间都必须加上分钟结尾。
`);
...
async function generateTeachingPlan() {
  ...
  try {
    ...
    jaObj.value = JSON.parse(processArrayAfterThink(textRz.value))
    isLoading.value = false;
...
<template>
  <div class="main-content">
    ...
    <div v-if="isLoading" class="loading-overlay">
      <div class="loading-container">
        ...
        <div class="loading-progress">
          ...
          <div class="progress-text">请稍候，这可能需要十几秒钟</div>
        </div>
      </div>
    </div>
    ...
    <div class="page-header">
      ...
      <div>
        <button class="btn btn-success" @click="exportTeachingPlan">
    ...
    <div class="teaching-plan-container">
      <div class="plan-summary">
        <div class="summary-card">
          ...
          <div class="summary-value">{{ String(jaObj.jxfa.lt).endsWith('分钟') ? jaObj.jxfa.lt : `${jaObj.jxfa.lt}分钟` }}</div>
        </div>
        ...
        <div class="summary-card theory">
          ...
          <div class="summary-value">{{ String(jaObj.jxfa.lljj).endsWith('分钟') ? jaObj.jxfa.lljj : `${jaObj.jxfa.lljj}分钟` }}</div>
        ...
        <div class="summary-card practice">
          ...
          <div class="summary-value">{{ String(jaObj.jxfa.sjcz).endsWith('分钟') ? jaObj.jxfa.sjcz : `${jaObj.jxfa.sjcz}分钟` }}</div>
        ...
        <div class="summary-card interaction">
          ...
          <div class="summary-value">{{ String(jaObj.jxfa.hdhj).endsWith('分钟') ? jaObj.jxfa.hdhj : `${jaObj.jxfa.hdhj}分钟` }}</div>
          ...
          <div class="timeline-item" v-for="item in jaObj.zsd">
            <div class="timeline-content">
              ...
              <div class="timeline-duration">
                <i class="fas fa-clock"></i> {{ String(item.sc).endsWith('分钟') ? item.sc : `${item.sc}分钟` }}
              </div>
              <ul>
        ...
        <div class="case-cards">
          <div class="case-card" v-for="item in jaObj.sxal">
            ...
            <div class="case-body">
              ...
              <div class="case-goal">
                <strong>实践目标：</strong>{{ Array.isArray(item.mb) ? item.mb.join('，') : item.mb }}
              </div>
              <ol class="case-steps">
